...
class Index extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        if( isset($_POST['payment_options']) && $_POST['payment_options'] != "" ) {
            ...
            //Get customer email
            $current_custemor_email = $quote->getBillingAddress()->getEmail();
            ...
            if ( $customerType == \Magento\Checkout\Model\Type\Onepage::METHOD_GUEST) {
                ...
                $quote->setCustomerId(null)
                    ->setCustomerEmail($current_custemor_email)
            ...
            $params = array(
                ...
                'mpstimer'        => '0',
                'mpstimerbox'     => '#counter',
                'mpscancelurl'    => $base_url.'sandboxseamless/'
            );

            $this->getResponse()->setBody(json_encode($params));
            
        }
        else if( isset($_POST[ 'mpsorderid' ]) && $_POST != "" ) {   // Get the return from MOLPay
        ...
        elseif( isset($_POST['status'] ) ) //response from MOLPay 
	    {             
            ...
            if($skey == $key1) {
                ...
                if($status == '00')  {   // Success Payment
                   ...
                   if ($order->getId() && $order->getState() != 'processing') {
                        ...
                        $order->addStatusHistoryComment(__('Response from MOLPay - '. $nbcb_type . ' (Transaction Status : CAPTURED)'))
